...
/// FeedbackJar SDK for iOS and macOS.
///
...
public final class FeedbackJar: @unchecked Sendable {
    ...
    private static let userIdDefaultsKey = "com.feedbackjar.sdk.identity.userId"
    private static let signatureDefaultsKey = "com.feedbackjar.sdk.identity.signature"
    private static let timestampDefaultsKey = "com.feedbackjar.sdk.identity.timestamp"
    private static let firstNameDefaultsKey = "com.feedbackjar.sdk.identity.firstName"
    private static let lastNameDefaultsKey = "com.feedbackjar.sdk.identity.lastName"
    private static let avatarDefaultsKey = "com.feedbackjar.sdk.identity.avatar"
    /// 7 days — must match `WIDGET_IDENTITY_MAX_AGE_MS` on the server.
    private static let identityMaxAgeMs = 7 * 24 * 60 * 60 * 1000
    ...
    /// never a fresh `Date()`, or the signature will fail verification. Expires
    /// after 7 days; call this again (e.g. on each sign-in) to refresh it.
    public func setIdentity(
        ...
        let defaults = UserDefaults.standard
        defaults.set(userId, forKey: Self.userIdDefaultsKey)
        defaults.set(email, forKey: Self.emailDefaultsKey)
        defaults.set(signature, forKey: Self.signatureDefaultsKey)
        defaults.set(timestamp, forKey: Self.timestampDefaultsKey)
        if let name { defaults.set(name, forKey: Self.nameDefaultsKey) }
        if let firstName { defaults.set(firstName, forKey: Self.firstNameDefaultsKey) }
        if let lastName { defaults.set(lastName, forKey: Self.lastNameDefaultsKey) }
        if let avatar { defaults.set(avatar, forKey: Self.avatarDefaultsKey) }
    }

    /// The currently remembered submitter identity, if any.
    public func getIdentity() -> FeedbackIdentity {
        let defaults = UserDefaults.standard
        let timestamp = defaults.integer(forKey: Self.timestampDefaultsKey)
        return FeedbackIdentity(
            ...
            userId: defaults.string(forKey: Self.userIdDefaultsKey),
            signature: defaults.string(forKey: Self.signatureDefaultsKey),
            timestamp: timestamp > 0 ? timestamp : nil,
            firstName: defaults.string(forKey: Self.firstNameDefaultsKey),
            lastName: defaults.string(forKey: Self.lastNameDefaultsKey),
            avatar: defaults.string(forKey: Self.avatarDefaultsKey)
        )
    }
    ...
    public func clearIdentity() {
        let defaults = UserDefaults.standard
        for key in [
            Self.nameDefaultsKey, Self.emailDefaultsKey, Self.userIdDefaultsKey,
            Self.signatureDefaultsKey, Self.timestampDefaultsKey,
            Self.firstNameDefaultsKey, Self.lastNameDefaultsKey, Self.avatarDefaultsKey,
        ] {
            defaults.removeObject(forKey: key)
        }
    }

    /// The stored identity as a wire-ready `SignedIdentity`, or nil when it isn't
    /// verified (no signature) or has aged past `identityMaxAgeMs` — callers then
    /// fall back to the anonymous id, same as if `setIdentity` was never called.
    private func verifiedIdentity() -> SignedIdentity? {
        let identity = getIdentity()
        guard
            let userId = identity.userId, !userId.isEmpty,
            let email = identity.email, !email.isEmpty,
            let signature = identity.signature, !signature.isEmpty,
            let timestamp = identity.timestamp
        else { return nil }

        let ageMs = Int(Date().timeIntervalSince1970 * 1000) - timestamp
        guard ageMs >= 0, ageMs <= Self.identityMaxAgeMs else { return nil }
    ...
    public func submit(_ content: String, email: String? = nil, name: String? = nil, properties: [String: Any]? = nil) async -> Result<FeedbackResponse, Error> {
        ...
        return await client.submit(widgetId: id, content: content, email: email ?? identity.email, name: name ?? identity.name, metadata: metadata, identity: verifiedIdentity())
    }
    ...
    public func listFeedback(
        ...
        guard let id = widgetId else { return .failure(FeedbackJarError.notInitialized) }
        guard let client else { return .failure(FeedbackJarError.notInitialized) }
        return await client.listFeedback(widgetId: id, boardId: boardId, limit: min(max(limit, 1), 50), cursor: cursor, identity: verifiedIdentity())
    }
    ...
    public func getPost(_ postId: String) async -> Result<FeedbackPost, Error> {
        ...
        return await client.getPost(widgetId: id, postId: postId, identity: verifiedIdentity())
    }
    ...
    public func vote(postId: String) async -> Result<VoteState, Error> {
        guard let id = widgetId, let client else { return .failure(FeedbackJarError.notInitialized) }
        return await client.vote(widgetId: id, postId: postId, identity: verifiedIdentity())
    }
    ...
    public func unvote(postId: String) async -> Result<VoteState, Error> {
        guard let id = widgetId, let client else { return .failure(FeedbackJarError.notInitialized) }
        return await client.unvote(widgetId: id, postId: postId, identity: verifiedIdentity())
    }
    ...
    public func voteState(postId: String) async -> Result<VoteState, Error> {
        guard let id = widgetId, let client else { return .failure(FeedbackJarError.notInitialized) }
        return await client.getVoteState(widgetId: id, postId: postId, identity: verifiedIdentity())
    }

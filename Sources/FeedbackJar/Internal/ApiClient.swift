...
internal struct SignedIdentity: Encodable {
    let userId: String
    let email: String
    let timestamp: Int
...
internal final class ApiClient: Sendable {
    ...
    private func applyIdentityHeader(to request: inout URLRequest, identity: SignedIdentity?) {
        guard let identity, let data = try? JSONEncoder().encode(identity) else { return }
        let encoded = data.base64EncodedString()
        request.setValue(encoded, forHTTPHeaderField: "X-FeedbackJar-Identity")
    }
    ...
    func submit(widgetId: String, content: String, email: String? = nil, name: String? = nil, metadata: DeviceMetadata, identity: SignedIdentity? = nil) async -> Result<FeedbackResponse, Error> {
        ...
        do {
            request.httpBody = try JSONEncoder().encode(SubmitRequest(content: content, email: email, userName: name, metadata: metadata, identity: identity))
    ...
    func listFeedback(widgetId: String, boardId: String?, limit: Int, cursor: String?, identity: SignedIdentity? = nil) async -> Result<FeedbackListResult, Error> {
        ...
        applyIdentityHeader(to: &request, identity: identity)
    ...
    func getPost(widgetId: String, postId: String, identity: SignedIdentity? = nil) async -> Result<FeedbackPost, Error> {
    ...
    func vote(widgetId: String, postId: String, identity: SignedIdentity? = nil) async -> Result<VoteState, Error> {
        await voteRequest(widgetId: widgetId, postId: postId, action: "vote", identity: identity)
    }

    func unvote(widgetId: String, postId: String, identity: SignedIdentity? = nil) async -> Result<VoteState, Error> {
        await voteRequest(widgetId: widgetId, postId: postId, action: "unvote", identity: identity)
    }

    private func voteRequest(widgetId: String, postId: String, action: String, identity: SignedIdentity?) async -> Result<VoteState, Error> {
    ...
    func getVoteState(widgetId: String, postId: String, identity: SignedIdentity? = nil) async -> Result<VoteState, Error> {
    ...
    func createComment(widgetId: String, postId: String, content: String, parentId: String?, name: String?, email: String?, identity: SignedIdentity? = nil) async -> Result<String, Error> {

...
A lightweight SDK for collecting user feedback on iOS and macOS. Drop in the prebuilt `FeedbackJarBoard` UI, or build your own form — the SDK handles submission (enriched with device metadata), fetching the public feedback list, and guest upvoting and commenting.
...
```swift
dependencies: [
    .package(url: "https://github.com/feedbackjar/swift-sdk", from: "1.4.0"),
],
targets: [
...
- No dependencies beyond the Swift standard library and `Foundation`/`UIKit` (or `AppKit` on macOS). The prebuilt UI uses `SwiftUI`, a system framework — still no third-party dependency.
